-- Allow premium users to review wrong/skipped objective answers after an attempt is completed.
-- Correct answers remain hidden until the user explicitly reveals a question.
...
begin
  ...
  select aq.id, qa.status, q.explanation
  into v_attempt_question_id, v_attempt_status, v_explanation
  from public.quiz_attempts qa
  join public.attempt_questions aq on aq.attempt_id = qa.id
  join public.questions q on q.id = aq.question_id
  where qa.id = p_attempt_id
    and qa.user_id = v_user_id
    and qa.status in ('in_progress', 'completed')
  ...
  if v_attempt_status = 'completed'
    and not (public.is_premium_user(v_user_id) or public.is_admin_user(v_user_id)) then
    ...
    'questions', coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'is_correct', case
            when aa.revealed_at is not null then coalesce(aa.is_correct, false)
            when qa.status = 'completed' and (public.is_premium_user(v_user_id) or public.is_admin_user(v_user_id)) then coalesce(aa.is_correct, false)
            else null
          end,
