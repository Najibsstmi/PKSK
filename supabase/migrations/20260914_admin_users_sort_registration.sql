...
begin
  ...
  order by
    case
      when filtered.subscription_status = 'premium'
        and filtered.is_blocked = false
        and (filtered.subscription_ends_at is null or filtered.subscription_ends_at > now())
        then 0
      when filtered.subscription_status = 'free'
        and filtered.is_blocked = false
        then 1
      else 2
    end,
    case
      when filtered.subscription_status = 'premium'
        and filtered.is_blocked = false
        and (filtered.subscription_ends_at is null or filtered.subscription_ends_at > now())
        then coalesce(filtered.subscription_started_at, filtered.created_at)
      else filtered.created_at
    end desc nulls last,
