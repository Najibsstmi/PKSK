...
function TopBar({
  ...
  const accountLabel = profile?.display_name ?? profile?.full_name ?? "Akaun";
  const homeRoute: AppRoute = isLoggedIn && access.canUsePremiumFeature() ? "/app" : "/";
  const accountRoute: AppRoute = access.canUsePremiumFeature() ? "/app/profile" : "/";
  const showPremiumCta = !access.canUsePremiumFeature() && currentRoute !== "/checkout" && currentRoute !== "/payment-result";
  const coreNavItems: Array<{ to: AppRoute; label: string; icon: LucideIcon }> = [
    { to: homeRoute, label: "Home", icon: LayoutDashboard },
    { to: "/info-pksk", label: "Info PKSK", icon: Info },
    { to: "/app/panduan", label: "Panduan", icon: BookOpen },
  ];
  const appNavItems =
    isLoggedIn && access.canUsePremiumFeature() && !isPublicShell
      ? navItems.filter((item) => {
          if (["/app", "/info-pksk", "/app/panduan"].includes(item.to)) {
            return false;
          }
      ...
      : [];
  const desktopNavItems = [...coreNavItems, ...appNavItems];

  const renderPremiumButton = (compact = false, label = "Dapatkan Premium", showBadge = true) => (
    <button
      type="button"
      onClick={onPremiumCheckout}
      className={`topbar-premium-button ${compact ? "min-h-12 w-full" : "h-11"}`}
    >
      {showBadge ? <span className="topbar-premium-badge">Paling Popular</span> : null}
      <Crown size={compact ? 17 : 16} aria-hidden="true" />
      {label}
    </button>
  );

  const renderNavButton = (item: { to: AppRoute; label: string; icon: LucideIcon; shortLabel?: string }, compact = false) => {
    const isActive = currentRoute === item.to || (item.to === homeRoute && ((homeRoute === "/app" && currentRoute === "/app") || (homeRoute === "/" && currentRoute === "/")));
    ...
    return (
      <button
        ...
        type="button"
        onClick={() => onNavigate(item.to)}
        className={`topbar-nav-button ${isActive ? "topbar-nav-button--active" : "topbar-nav-button--idle"}`}
        aria-current={isActive ? "page" : undefined}
        aria-label={item.label}
        title={item.label}
      >
        <Icon className="topbar-nav-icon" size={17} aria-hidden="true" />
        <span className="topbar-nav-label">{item.shortLabel ?? item.label}</span>
      </button>
    );
  };
    ...
    <div className="fixed inset-x-0 top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="topbar-main-nav hidden lg:flex" aria-label="Navigasi utama">
          {desktopNavItems.map((item) => renderNavButton(item))}
        </nav>

        <div className="hidden items-center gap-3 lg:flex">
          {showPremiumCta ? renderPremiumButton() : null}
          {isLoggedIn ? (
            <>
              <button
                type="button"
                onClick={() => onNavigate(accountRoute)}
                className="topbar-account-button"
                title="Akaun sedang log masuk"
              >
        ...
        <div className="ml-auto flex items-center gap-2 lg:hidden">
          {showPremiumCta ? (
            <button type="button" onClick={onPremiumCheckout} className="topbar-mobile-premium-button">
      ...
      {isMenuOpen ? (
        <nav className="border-t border-slate-100 bg-white px-4 py-3 lg:hidden" aria-label="Navigasi mudah alih">
          <div className="mx-auto grid max-w-7xl gap-2">
            {desktopNavItems.map((item) => renderNavButton(item, true))}
            {showPremiumCta ? renderPremiumButton(true) : null}
            {isLoggedIn ? (
              <button type="button" onClick={() => onNavigate(accountRoute)} className="topbar-login-button h-12 w-full rounded-xl" title="Akaun sedang log masuk">
                <UserRound size={17} aria-hidden="true" />
                {accountLabel}
              </button>
            ) : null}
...
function AchievementsPage({
  ...
  return (
    <div className="space-y-6">
      <section className="badge-stage simulation-stage relative overflow-hidden rounded-2xl p-4 text-white shadow-soft sm:p-8">
        <div className="simulation-stage__beam" aria-hidden="true" />
        <div className="relative z-10 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            <span className="inline-flex w-fit items-center gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs font-black uppercase text-cyan-50 ring-1 ring-white/20">
            ...
            <h1 className="mt-3 text-2xl font-black leading-tight text-white sm:mt-4 sm:text-4xl">Lencana</h1>
            <p className="mt-2 max-w-2xl text-[13px] font-semibold leading-6 text-cyan-50/90 sm:mt-3 sm:text-base">
              Kumpul lencana apabila berjaya mencapai sasaran simulasi dan lihat kemajuan setiap misi.
            </p>
            <div className="badge-stage__stats mt-4 flex flex-wrap gap-2">
              <span>{earnedBadges} / {totalBadges} dibuka</span>
              <span>{nextBadge ? `Seterusnya: ${nextBadge.name}` : "Semua lencana sudah dibuka"}</span>
            </div>
          </div>
          <div className="simulation-stage__radar" aria-hidden="true">
            <span />
      ...
      <section className="badge-showcase-grid grid" aria-label="Koleksi lencana PKSK">
        {badges.map((badge) => {
          const Icon = iconForBadge(badge.icon);
          const percentage = Math.min(100, Math.round((badge.progress_value / Number(badge.requirement_value || 1)) * 100));
          return (
            <article
              key={badge.id}
              className={`badge-showcase-card badge-showcase-card--${badge.tier.toLowerCase()} ${
                badge.earned ? "badge-showcase-card--earned" : "badge-showcase-card--locked"
              } relative overflow-hidden rounded-2xl border p-4 text-white shadow-soft`}
            >
              <div className="badge-showcase-card__noise" aria-hidden="true" />
              <div className="badge-showcase-card__art" aria-hidden="true">
                <span className="badge-showcase-card__art-ring badge-showcase-card__art-ring--outer" />
                <span className="badge-showcase-card__art-ring badge-showcase-card__art-ring--inner" />
                <span className="badge-showcase-card__art-spark badge-showcase-card__art-spark--one" />
                <span className="badge-showcase-card__art-spark badge-showcase-card__art-spark--two" />
                <span className="badge-showcase-card__art-medal">
                  <Icon size={52} strokeWidth={2.2} />
                </span>
                {!badge.earned ? (
                  <span className="badge-showcase-card__art-lock">
                    <LockKeyhole size={15} strokeWidth={2.7} />
                  </span>
                ) : null}
              </div>
              <div className="relative z-10 flex h-full min-w-0 flex-col">
                <div className="flex items-start justify-between gap-2">
                  <span className="badge-showcase-card__seal grid h-12 w-12 shrink-0 place-items-center rounded-2xl">
                    {badge.earned ? <Icon size={24} strokeWidth={2.5} aria-hidden="true" /> : <LockKeyhole size={24} strokeWidth={2.5} aria-hidden="true" />}
                  </span>
                  <span className="badge-showcase-card__tier rounded-full px-2.5 py-1 text-[10px] font-black uppercase">
                    {tierLabel(badge.tier)}
                  </span>
                </div>

                <div className="badge-showcase-card__copy mt-4 min-w-0">
                  <h2 className="badge-showcase-card__title text-lg font-black leading-tight text-white">{badge.name}</h2>
                  <p className="badge-showcase-card__description mt-2 text-xs font-semibold leading-5 text-white/78">{badge.description}</p>
                </div>

                <div className="mt-auto pt-4">
                  <div className="mb-2 flex items-center justify-between gap-2 text-[11px] font-black text-white/72">
                    <span>Kemajuan</span>
                    <span>{badge.progress_label}</span>
                  </div>
                  <div className="badge-showcase-card__progress h-2 overflow-hidden rounded-full bg-white/18">
                    <div className="h-full rounded-full" style={{ width: `${percentage}%` }} />
                  </div>
                  <div className="mt-3 flex items-center justify-between gap-2">
                    <span className="badge-showcase-card__status inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-black uppercase">
                      {badge.earned ? (
                        <>
                    ...
                    {badge.earned_at ? <span className="badge-showcase-card__date text-[10px] font-bold text-white/62">{formatDate(badge.earned_at)}</span> : null}
                  </div>
                </div>
              </div>
            </article>
